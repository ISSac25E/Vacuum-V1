  ...
  //MQTT:
  {
    ...
    if (!MQTT_Connected) {
      ...
      if (CLIENT.connect(VACUUM_MQTT_CLIENT_NAME, VACUUM_MQTT_USERNAME, VACUUM_MQTT_PASSWORD)) {
        CLIENT.publish(VACUUM_MQTT_CLIENT_NAME"/checkIn", "Rebooted");    //TODO: Try "OK" instead, Maybe this is why we are getting ureliable connection?
...
void MQTT_UpdateStatus() {
  if (MQTT_Connected) {
    ...
    if (UpdateStatus || millis() - UpdateStatusTimer >= 300) {
      ...
      {
        String TempStr = String((millis() / 60) / 60) + "h:" + String((millis() / 60) % 60) + "m:" + String(millis() % 60) + "s";
        char CharPayLoad[50];
        TempStr.toCharArray(CharPayLoad, TempStr.length() + 1);
