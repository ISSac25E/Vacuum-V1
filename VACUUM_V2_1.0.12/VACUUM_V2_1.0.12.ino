...
//MQTT Server Setup:
#define VACUUM_MQTT_SERVER_IP "192.168.1.100"
...
void RunErrorButton()
{
  ...
  static bool ErrorButtonEnable = true;
  //Check if Vacuum is not Charging and Enable Button:
  if (!MPU_Moving && WHITE_IN_STATE == 2) //Vacuum Charging
  {
    ErrorButtonEnable = false;
  }
  else if (MPU_Moving && WHITE_IN_STATE != 2)
  {
    ErrorButtonEnable = true;
  }
  //Only Run Max every 5 Seconds:
  if (millis() - ErrorButtonTimeOut >= 5000)
  {
    if (ErrorButtonEnable && ORANGE_IN_STATE)
    {
