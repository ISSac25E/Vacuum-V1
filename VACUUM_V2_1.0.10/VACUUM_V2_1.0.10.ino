...
//WIFI Network Setup:
#define VACUUM_NETWORK_SSID "SSID"     // <<<<<< Make Sure to update your SSID
#define VACUUM_NETWORK_PASSWORD "PASS" // <<<<<< Make Sure to upadate your Password

/*                     ▼▼▼▼▼▼▼▼▼▼ I don't know if anything else Changed ▼▼▼▼▼▼▼▼▼▼                          */

//MQTT Server Setup:
#define VACUUM_MQTT_SERVER_IP "10.157.1.100" // <<<<<< HomeAssistant Ip Changed?
...
uint8_t ORANGE_BLINK_COUNT = 0; //Check how many times the orange LED Blinks
...
void LED_Run()
{
  //LED Update:
  {
    ...
    //WHITE LED:
    {
      ...
      {
        if (WHITE_IN_MACRO.PrevInterval() < LED_THRES)
          WHITE_IN_STATE = 2; //LED Blinking
        else
        {
          if (WHITE_IN_MACRO.State())
          {
            WHITE_IN_STATE = 1; //LED ON
          }
          else
          {
            WHITE_IN_STATE = 0; //LED OFF
          }
        }
      }
    ...
    //ORANGE LED:
    {
      ...
      {
        if (ORANGE_IN_MACRO.PrevInterval() < LED_THRES)
        {
          ORANGE_IN_STATE = 2; //LED Blinking
          ORANGE_BLINK_COUNT++;
        }
        else
        {
          ORANGE_BLINK_COUNT = 0;
          if (ORANGE_IN_MACRO.State())
          {
            ORANGE_IN_STATE = 1; //LED ON
          }
          else
          {
            ORANGE_IN_STATE = 0; //LED OFF
          }
        }
      }
...
void RunErrorButton()
{
  ...
  if (millis() - ErrorButtonTimeOut >= 5000)
  {
    if ((ORANGE_IN_STATE && MPU_Moving) || (ORANGE_IN_STATE == 2 && ORANGE_BLINK_COUNT > 1))
    {
...
void MQTT_UpdateStatus()
{
  if (MQTT_Connected)
  {
    ...
    if (UpdateStatus || millis() - UpdateStatusTimer >= 300)
    {
      ...
      if (VACUUM_MPU.Error())
        CLIENT.publish(VACUUM_MQTT_CLIENT_NAME "/Status", "GYRO ERROR");
      else if (VACUUM_MPU.CalcOffsets)
        CLIENT.publish(VACUUM_MQTT_CLIENT_NAME "/Status", "GYRO CALIBRATING");
      else
      {
        if (WHITE_IN_STATE > 0)
        {
        ...
        {
          if (MPU_Moving)
            CLIENT.publish(VACUUM_MQTT_CLIENT_NAME "/Status", "MOVING");
          else
            CLIENT.publish(VACUUM_MQTT_CLIENT_NAME "/Status", "NOT MOVING");
        }
      }
